const anzhiyu = {
  ...
  getCustomPlayList: function () {
    if (!window.location.pathname.startsWith("/preview/music/")) {
      return;
    }
  ...
  addEventListenerMusic: function () {
    ...
    function anMusicPageMenuAask() {
      if (window.location.pathname != "/preview/music/") {
  ...
  changeMusicList: async function () { return anzhiyu.getCustomPlayList();
    ...
    if (changeMusicListFlag) {
      ...
      if (currentTime - cacheData.timestamp < 24 * 60 * 60 * 1000) {
        ...
        // 否则重新从服务器获取数据
        const response = await fetch("/preview/json/music.json");
  ...
  toPage: function () {
    ...
    if (!isNaN(pageNumber) && pageNumber >= 1 && Number.isInteger(pageNumber)) {
      var url = "/preview/page/" + (pageNumber > lastPageNumber ? lastPageNumber : pageNumber) + "/";
      toPageButton.href = pageNumber === 1 ? "/preview/" : url;
    } else {
