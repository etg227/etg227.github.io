...
function uniqueSubs(sub){const seen=new Set(),out=[];(sub||[]).forEach(x=>{if(x.type&&x.value&&!seen.has(x.type)){seen.add(x.type);out.push(x)}});return out}
function echoLines(e){const out=[],f=FIXED_MAIN[e.cost];if(e.mainType)out.push({type:e.mainType,value:MAIN_VALUE[e.cost]?.[e.mainType]||0});out.push({...f});uniqueSubs(e.sub).forEach(x=>out.push(x));return out}
...
// agg 为当前被评估的声骸聚合（候选替换时是候选配置），条件型套装效果按它重新判定。
function sonataEffects(ctx,agg){const pieces=$('sonataPieces')?.value||'none',main=setByName($('sonataMain')?.value),sec=setByName($('sonataSecondary')?.value),out=[];const echoEr=(agg||ctx.curAgg).energyRegen||0;const add=(s,p)=>{(s?.[`p${p}`]||[]).forEach(e=>{if(e.condition==='er250'){const er=ctx.nonEcho.energyRegen+echoEr+(s.p2||[]).filter(z=>z.kind==='energyRegen').reduce((n,z)=>n+(+z.value||0),0);if(er<250)return}out.push(e)})};if(pieces==='5'){add(main,2);add(main,5)}else if(pieces==='3'){add(main,3);add(sec,2)}else if(pieces==='1'){add(main,1)}return out}
function autoEffects(ctx,agg){const out=[...chainEffects(ctx.profile,ctx.chain),...sonataEffects(ctx,agg)];if($('weaponMode')?.value==='signature')out.push(...(ctx.profile?.signature_weapon?.effects||[]));return out}
function bucket(ctx,agg){const a={atkPct:0,hpPct:0,defPct:0,critRate:0,critDmg:0,energyRegen:0,elementDmg:0,globalDmg:0,globalAmp:0,basicDmg:0,heavyDmg:0,skillDmg:0,liberationDmg:0};autoEffects(ctx,agg).forEach(e=>{const k=e.kind||e.type;if(k in a)a[k]+=+e.value||0});return a}
function model(ctx,a){const z=bucket(ctx,a);return{atk:ctx.baseEff.atk*(1+(a.atkPct+z.atkPct+ctx.panel.nonEchoAtkPct)/100)+a.flatAtk,hp:ctx.baseEff.hp*(1+(a.hpPct+z.hpPct+ctx.panel.nonEchoHpPct)/100)+a.flatHp,def:ctx.baseEff.def*(1+(a.defPct+z.defPct+ctx.panel.nonEchoDefPct)/100)+a.flatDef,critRate:ctx.nonEcho.critRate+a.critRate+z.critRate,critDmg:ctx.nonEcho.critDmg+a.critDmg+z.critDmg,energyRegen:ctx.nonEcho.energyRegen+a.energyRegen+z.energyRegen,elementDmg:ctx.nonEcho.extraElementDmg+a.elementDmg+z.elementDmg,globalDmg:ctx.panel.globalDmg+z.globalDmg,globalAmp:ctx.panel.globalAmp+z.globalAmp,typeDmg:{basic:a.basicDmg+z.basicDmg,heavy:a.heavyDmg+z.heavyDmg,skill:a.skillDmg+z.skillDmg,liberation:a.liberationDmg+z.liberationDmg}}}
...
function scoreEcho(ctx,e){
  ...
  if(!(cap>0))return 0;
  let s=0;uniqueSubs(e.sub).forEach(l=>{s+=(+l.value)*(c[l.type]||0)});
  return s/cap*100;
}
...
function echoPoints(e,c){
  ...
  uniqueSubs(e.sub).forEach(l=>{s+=(+l.value)*(c[l.type]||0)});
  return s;
}
...
// 同一只声骸内副词条类型不可重复（与游戏规则一致）；重复选择时清空并闪烁提示。
function enforceUniqueSub(sel,scope){
  if(!sel.value||!scope)return true;
  const dup=[...scope.querySelectorAll('.echo-row .echo-type')].some(s=>s!==sel&&s.value===sel.value);
  if(!dup)return true;
  sel.value='';
  sel.classList.add('dup-flash');
  setTimeout(()=>sel.classList.remove('dup-flash'),900);
  return false;
}
function costCheck(slot){
  const total=M.echoes.reduce((s,e)=>s+(+e.cost||0),0);
  const next=total-(+M.echoes[slot]?.cost||0)+(+M.candidate.cost||0);
  const el=$('costWarning');if(!el)return;
  if(total>12){el.hidden=false;el.textContent=`⚠ 当前总 COST ${total}/12 超出游戏上限，该配置在游戏内不可用，结果仅供理论参考。`}
  else if(next>12){el.hidden=false;el.textContent=`⚠ 替换后总 COST 将达 ${next}/12，超出游戏上限，请调整候选声骸的 COST。`}
  else el.hidden=true;
}
...
function renderProfilePanel(ctx){
  ...
  const fx=autoEffects(ctx,ctx.curAgg);
...
// 可实现建议：每个方案都克隆整套声骸做完整重算（含暴击上限、套装阈值等边界），不再按比例近似。
// 提档 = 已有该词条的位置提到满档；替换 = 无该词条的声骸，将其某条现有词条换成该词条的平均档（取该声骸内最优替换位）。
function actionable(ctx,std){
  const base=factorSet(ctx,M.echoes);if(!(base>0))return[];
  const out=[];
  for(const x of std){
    const rolls=ROLLS[x.type]||[];if(!rolls.length)continue;
    const max=Math.max(...rolls),av=avg(rolls);
    let best=null;
    M.echoes.forEach((e,i)=>{
      const idx=e.sub.findIndex(l=>l.type===x.type&&l.value);
      if(idx>=0){
        if(max-(+e.sub[idx].value||0)<=1e-6)return;
        const next=clone(M.echoes);next[i].sub[idx]={type:x.type,value:max};
        const g=(factorSet(ctx,next)/base-1)*100;
        if(g>1e-4&&(!best||g>best.gain))best={type:x.type,gain:g,slot:i,mode:'upgrade'};
      }else{
        let lg=null;
        e.sub.forEach((l,j)=>{
          if(!l.type||!l.value)return;
          const next=clone(M.echoes);next[i].sub[j]={type:x.type,value:av};
          const g=(factorSet(ctx,next)/base-1)*100;
          if(lg===null||g>lg)lg=g;
        });
        if(lg!==null&&lg>1e-4&&(!best||lg>best.gain))best={type:x.type,gain:lg,slot:i,mode:'replace'};
      }
    });
