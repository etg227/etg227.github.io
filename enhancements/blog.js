(() => {
  ...
  function setup() {
    ...
    if (nav && !document.getElementById('flower-search-link')) {
      ...
      a.id = 'flower-search-link'; a.href = '/search/'; a.textContent = '搜索';
    ...
    fetch('/enhancements/search.json').then(r => {if(!r.ok) throw Error('index');return r.json();}).then(data => {posts=data;render();}).catch(() => {status.textContent='搜索暂时无法加载，请刷新重试。';});
  }
